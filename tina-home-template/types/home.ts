/**
 * Content shapes for the home collection, mirroring tina/home-collection.ts.
 *
 * Tina generates every field as nullable, so these are all optional — the
 * components must render sensibly with missing values, and they do.
 *
 * Kept hand-written rather than importing Tina's generated types so the
 * template compiles before `tinacms build` has ever run. Once the collection
 * is wired up you can switch to the generated types if you prefer; the shapes
 * match.
 */
...
/** Labels only — the quoting logic itself lives in the component. */
...
export type FeatureItem = {
  ...
  body?: Maybe<string>;
  icon?: Maybe<string>;
  /** Only the featured card renders this. */
  image?: Maybe<string>;
  ctaLabel?: Maybe<string>;
...
export type FeaturesContent = {
  ...
  /** Desktop bento grid — styled by position, see DESKTOP_SLOTS. */
  items?: Maybe<Maybe<FeatureItem>[]>;
  /** Mobile cards — a reduced set, no image or CTA. */
...
export type FaqItem = {
  ...
  /** Hides the entry on desktop rather than dropping it entirely. */
